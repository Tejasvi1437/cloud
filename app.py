        ...
        <ul id="taskList">
            {% for task in tasks %}
                <li class="{{ 'completed' if task['completed'] else '' }}" id="task-{{ loop.index0 }}">
                    {{ task['text'] }}
                    <button class="removeButton" data-id="{{ loop.index0 }}">❌</button>
                    <button class="toggleButton" data-id="{{ loop.index0 }}">✔️</button>
                </li>
            {% endfor %}
        </ul>
    </div>
        ...
        function updateTaskList(tasks) {
            ...
            tasks.forEach((task, index) => {
                ...
                li.innerHTML = `${task.text} 
                                <button class="removeButton" data-id="${index}">❌</button> 
                                <button class="toggleButton" data-id="${index}">✔️</button>`;
